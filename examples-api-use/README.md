...
Options:
        ...
        --led-pixel-mapper        : Semicolon-separated list of pixel-mappers to arrange pixels.
                                    ...
                                    Available: "Mirror", "Rotate", "Rotate-panel", "U-mapper", "V-mapper", "Row-mapper", "Reorder". Default: ""
...
The "Rotate-panel" mapper allow you to rotate specific panel screens.
It takes a panel id and an angle as parameter after a colon, and multiple
panels are seperated by comma:

```
  ./demo --led-pixel-mapper="Rotate-panel:0|90,2|270"
```

#### Reorder

The "Reorder" mapper allow you to change the order of panels.
It takes two panel ids, the first id is the index of the panel to move and the
second id is the new index of the panel. Multiple panels are seperated by comma:

```
  ./demo --led-pixel-mapper="Reorder:1|3,3|1"
```
...
The 'Row-mapper' put panels in a single row, that are normally divided into multiple rows when using parallel chains.

```
6 panels, in 3 parallel chains with 2 in each chain (--led-chain=2 --led-parallel=3) normally looks like this:
   [<][<]  }--- Parallel connector #1
   [<][<]  }--- Parallel connector #2
   [<][<]  }--- Parallel connector #3

It will look like this, when using the Row-mapper:
   [<][<][<][<][<][<]
```

This is useful if you are using a hat/adapter with multiple parallel chains but trying to run code that was made for a single chain.
